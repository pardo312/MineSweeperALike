...
public class GameplayDataController : DataControllerBase
{
    #region <<<Init>>>
    ...
    {
        if (dataToSave.GetType() == typeof(BoardSaveData))
        {
            BoardSaveData boardData = (BoardSaveData)dataToSave;
    ...
    private void LoadBoardData(object payload, Action<DataResponseModel> callback)
    {
        ...
        try
        {
            boardData = JsonUtility.FromJson<BoardSaveData>(boardDataJson);
        }
        catch (Exception e)
        {
